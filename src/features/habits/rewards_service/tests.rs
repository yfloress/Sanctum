...
fn test_create_streak_reward_consecutive() {
    ...
    let id = h.service.create_streak_reward(habit_id.clone(), true, Some(7), None)
...
fn test_create_streak_reward_accumulative() {
    ...
    let id = h.service.create_streak_reward(habit_id, false, Some(30), Some(30))
        .expect("create reward");
    let reward = h.service.get_streak_reward(&id).expect("get reward");
    assert!(reward.is_some());
    let r = reward.unwrap();
...
fn test_get_streak_rewards_returns_all() {
    ...
    h.service.create_streak_reward(habit_id.clone(), true, Some(3), None).expect("r1");
    h.service.create_streak_reward(habit_id, false, Some(5), Some(5)).expect("r2");
    let rewards = h.service.get_streak_rewards().expect("get rewards");
...
fn test_get_streak_rewards_by_habit_filters() {
    ...
    h.service.create_streak_reward(h1.clone(), true, Some(3), None).expect("r1");
    h.service.create_streak_reward(h1.clone(), true, Some(7), None).expect("r2");
    h.service.create_streak_reward(h2, true, Some(5), None).expect("r3");
...
fn test_delete_streak_reward_removes_it() {
    ...
    let id = h.service.create_streak_reward(habit_id, true, Some(3), None).expect("create");
...
fn test_add_milestone_to_reward() {
    ...
    let reward_id = h.service.create_streak_reward(habit_id, true, Some(30), None).expect("create");
    let m_id = h.service.add_milestone(reward_id.clone(), 7, "First week!".to_string())
        .expect("add milestone");
    assert!(!m_id.is_empty());
    let milestones = h.service.get_milestones(&reward_id).expect("get milestones");
...
fn test_check_and_unlock_milestones_returns_empty_when_no_progress() {
    ...
    let reward_id = h.service.create_streak_reward(habit_id, true, Some(30), None).expect("create");
    h.service.add_milestone(reward_id.clone(), 5, "5 days".to_string()).expect("add m");
    h.service.add_milestone(reward_id.clone(), 10, "10 days".to_string()).expect("add m");
    let unlocked = h.service.check_and_unlock_milestones(&reward_id).expect("check");
...
fn test_check_and_unlock_milestones_unlocks_reached_milestones() {
    ...
    let milestones = h.service.get_milestones(&reward_id).expect("get milestones");
    let m5 = milestones.iter().find(|m| m.target_days == 5).expect("find 5-day");
    let m10 = milestones.iter().find(|m| m.target_days == 10).expect("find 10-day");
    assert!(m5.unlocked);
    assert!(!m10.unlocked);
}
...
fn test_update_streak_reward_with_milestones_creates_new_milestones() {
    ...
    let reward_id = h.service.create_streak_reward(habit_id.clone(), true, Some(30), None).expect("create");
    let milestones = vec![(7, "Week 1".to_string()), (14, "Week 2".to_string())];
    h.service.update_streak_reward_with_milestones(
        reward_id.clone(), habit_id, true, Some(30), None, milestones,
    ).expect("update");
    let ms = h.service.get_milestones(&reward_id).expect("get milestones");
    assert_eq!(ms.len(), 2);
}
...
fn test_update_streak_reward_with_milestones_replaces_existing() {
    ...
    let reward_id = h.service.create_streak_reward(habit_id.clone(), true, Some(30), None).expect("create");
    h.service.add_milestone(reward_id.clone(), 7, "Old".to_string()).expect("add");
    let new_ms = vec![(14, "New".to_string())];
    h.service.update_streak_reward_with_milestones(
        reward_id.clone(), habit_id, true, Some(30), None, new_ms,
    ).expect("update");
    let ms = h.service.get_milestones(&reward_id).expect("get milestones");
    assert_eq!(ms.len(), 1);
...
fn test_update_streak_reward_with_milestones_preserves_unlocked() {
    ...
    let reward_id = h.service.create_streak_reward(habit_id.clone(), true, Some(30), None).expect("create");
    h.service.add_milestone(reward_id.clone(), 5, "5 days".to_string()).expect("add");
    h.service.check_and_unlock_milestones(&reward_id).expect("check");
    ...
    let m10 = ms.iter().find(|m| m.target_days == 10).expect("find 10-day");
    assert!(m5.unlocked, "previously unlocked milestone should stay unlocked");
    assert!(m5.reward_text.contains("Updated"), "text should be updated");
    assert!(!m10.unlocked, "new milestone with target > progress should not unlock");
}
...
fn test_create_goal_minimal() {
    let h = new_harness();
    let id = h.service.create_goal("Minimal".to_string(), None, "Badge".to_string(), None)
        .expect("create goal");
...
fn test_get_goals_returns_all() {
    let h = new_harness();
    h.service.create_goal("Goal A".to_string(), None, "Badge".to_string(), None).expect("create");
    h.service.create_goal("Goal B".to_string(), None, "Badge".to_string(), None).expect("create");
    let goals = h.service.get_goals().expect("get goals");
...
fn test_update_goal_changes_fields() {
    let h = new_harness();
    let id = h.service.create_goal("Old".to_string(), None, "Old badge".to_string(), None)
        .expect("create");
    h.service.update_goal(
        id.clone(),
        "New".to_string(),
        "New desc".to_string(),
        "New badge".to_string(),
...
fn test_archive_goal() {
    let h = new_harness();
    let id = h.service.create_goal("Archivable".to_string(), None, "Badge".to_string(), None)
        .expect("create");
...
fn test_delete_goal_removes_it() {
    let h = new_harness();
    let id = h.service.create_goal("Deletable".to_string(), None, "Badge".to_string(), None)
        .expect("create");
...
fn test_complete_goal_creates_achievement() {
    let h = new_harness();
    let id = h.service.create_goal("Finish project".to_string(), None, "Ship it".to_string(), None)
        .expect("create");
...
fn test_complete_goal_twice_returns_none() {
    let h = new_harness();
    let id = h.service.create_goal("One time".to_string(), None, "Badge".to_string(), None)
        .expect("create");
...
fn test_add_checkpoint_auto_increments_order() {
    let h = new_harness();
    let goal_id = h.service.create_goal("Goal".to_string(), None, "Badge".to_string(), None)
        .expect("create");
    let c1 = h.service.add_checkpoint(goal_id.clone(), "Step 1".to_string()).expect("add c1");
    let c2 = h.service.add_checkpoint(goal_id.clone(), "Step 2".to_string()).expect("add c2");
    let cps = h.service.get_checkpoints(&goal_id).expect("get checkpoints");
    assert_eq!(cps.len(), 2);
...
fn test_delete_checkpoint_removes_it() {
    ...
    let goal_id = h.service.create_goal("Goal".to_string(), None, "Badge".to_string(), None)
        .expect("create");
    let cp_id = h.service.add_checkpoint(goal_id.clone(), "Temp".to_string()).expect("add");
...
fn test_update_checkpoint_changes_description() {
    ...
    let goal_id = h.service.create_goal("Goal".to_string(), None, "Badge".to_string(), None)
        .expect("create");
    let cp_id = h.service.add_checkpoint(goal_id, "Old desc".to_string()).expect("add");
    h.service.update_checkpoint(cp_id.clone(), "New desc".to_string()).expect("update");
    let cps = h.service.get_checkpoints(&h.service.get_goals().expect("goals")[0].id)
...
fn test_toggle_checkpoint_toggles_completion() {
    ...
    let goal_id = h.service.create_goal("Goal".to_string(), None, "Badge".to_string(), None)
        .expect("create");
    let cp_id = h.service.add_checkpoint(goal_id.clone(), "Step".to_string()).expect("add");
    let result = h.service.toggle_checkpoint(goal_id.clone(), cp_id.clone()).expect("toggle on");
    assert!(result, "first toggle should complete");
    let result = h.service.toggle_checkpoint(goal_id, cp_id.clone()).expect("toggle off");
...
fn test_toggle_checkpoint_not_found_returns_false() {
    let h = new_harness();
    let result = h.service.toggle_checkpoint("nonexistent".to_string(), "bad-id".to_string())
        .expect("toggle");
    assert!(!result);
}
...
fn test_toggle_checkpoint_completes_goal_when_all_done() {
    let h = new_harness();
    let goal_id = h.service.create_goal("Multi-step".to_string(), None, "Badge".to_string(), None)
        .expect("create");
    let c1 = h.service.add_checkpoint(goal_id.clone(), "Step 1".to_string()).expect("add c1");
    let c2 = h.service.add_checkpoint(goal_id.clone(), "Step 2".to_string()).expect("add c2");
    h.service.toggle_checkpoint(goal_id.clone(), c1).expect("complete c1");
    h.service.toggle_checkpoint(goal_id.clone(), c2).expect("complete c2");
    let goal = h.service.get_goal(&goal_id).expect("get").expect("found");
    assert!(goal.is_completed, "goal should auto-complete when all checkpoints done");
    let achievements = h.service.get_achievements().expect("get achievements");
    assert!(achievements.iter().any(|a| a.source_id == goal_id),
...
fn test_get_checkpoint_progress_tracks_counts() {
    ...
    let goal_id = h.service.create_goal("Goal".to_string(), None, "Badge".to_string(), None)
        .expect("create");
    let c1 = h.service.add_checkpoint(goal_id.clone(), "A".to_string()).expect("add");
    let c2 = h.service.add_checkpoint(goal_id.clone(), "B".to_string()).expect("add");
    let (total, completed) = h.service.get_checkpoint_progress(&goal_id).expect("progress");
    assert_eq!(total, 2);
    assert_eq!(completed, 0);
    h.service.toggle_checkpoint(goal_id.clone(), c1).expect("complete one");
    let (_, completed) = h.service.get_checkpoint_progress(&goal_id).expect("progress");
    assert_eq!(completed, 1);
    h.service.toggle_checkpoint(goal_id, c2).expect("complete two");
    let (total, completed) = h.service.get_checkpoint_progress(
        &h.service.get_goals().expect("goals")[0].id
    ).expect("progress");
...
fn test_achievements_created_via_goal_completion() {
    let h = new_harness();
    let id = h.service.create_goal("Achieve".to_string(), None, "Trophy".to_string(), None)
        .expect("create");
...
fn test_achievements_dedup_on_repeat_completion() {
    let h = new_harness();
    let id = h.service.create_goal("Unique".to_string(), None, "Badge".to_string(), None)
        .expect("create");
    h.service.complete_goal(id.clone()).expect("first");
    h.service.complete_goal(id).expect("second"); // should be deduped
    let achievements = h.service.get_achievements().expect("get");
    assert_eq!(achievements.len(), 1, "should only create one achievement per goal");
}
...
fn test_update_goal_with_checkpoints_replaces_checkpoints() {
    ...
    let goal_id = h.service.create_goal("Goal".to_string(), None, "Badge".to_string(), None)
        .expect("create");
    let old_cp = h.service.add_checkpoint(goal_id.clone(), "Old".to_string()).expect("add old");
    let new_checkpoints = vec![
        (Some(old_cp.clone()), "Updated".to_string(), 0),
        (None, "New Step".to_string(), 1),
    ];
    h.service.update_goal_with_checkpoints(
        goal_id.clone(),
