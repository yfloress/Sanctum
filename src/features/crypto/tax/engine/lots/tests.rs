...
#[test]
fn swap_fee_in_stablecoin_uses_usd_unit_price() {
    let mut report = base_report();
    let mut lots: HashMap<String, Vec<Lot>> = HashMap::new();
    let period = test_period();
    let ipc_map = BTreeMap::new();
    ...
    let mut tx = tx("s1", "swap", 45.6, 0.0109, "2024-02-10");
    tx.coin_id = "usdt".to_string();
    tx.symbol = "USDT".to_string();
    tx.fee_coin_id = Some("usdt".to_string());
    tx.fee_amount = Some(0.5);

    apply_fee_disposal(&mut report, &mut lots, &cfg, &tx);

    assert_eq!(report.disposals.len(), 1);
    assert!(approx_eq(report.disposals[0].proceeds, 0.5));
    assert!(
        !report
            .warnings
            .iter()
            .any(|w| w.code == "fee_missing_price")
    );
}

#[test]
fn swap_fee_in_non_pricing_asset_requires_explicit_usd_price() {
    let mut report = base_report();
    let mut lots: HashMap<String, Vec<Lot>> = HashMap::new();
    let period = test_period();
    let ipc_map = BTreeMap::new();
    ...
    let mut tx = tx("s1", "swap", 0.2, 14.0, "2024-02-10");
    tx.fee_coin_id = Some("btc".to_string());
    tx.fee_amount = Some(0.01);
...
#[test]
fn swap_fee_uses_fee_usd_when_fee_coin_price_is_missing() {
    let mut report = base_report();
    let mut lots: HashMap<String, Vec<Lot>> = HashMap::new();
    let period = test_period();
    let ipc_map = BTreeMap::new();
    ...
    let mut tx = tx("s2", "swap", 200.0, 1.0, "2024-02-10");
    tx.coin_id = "usdt".to_string();
    tx.symbol = "USDT".to_string();
    tx.fee_coin_id = Some("btc".to_string());
    tx.fee_amount = Some(0.0001);
    tx.fee = Some(6.5);

    apply_fee_disposal(&mut report, &mut lots, &cfg, &tx);

    assert_eq!(report.disposals.len(), 1);
    assert!(approx_eq(report.disposals[0].proceeds, 6.5));
    assert!(
        !report
            .warnings
            .iter()
