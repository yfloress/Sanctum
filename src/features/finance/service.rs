...
const EXCHANGE_RATE_TTL_SECS: i64 = 6 * 60 * 60;
...
impl FinanceService {
    ...
    pub fn create_account(
        ...
        self.with_db(|db| {
            ...
            if currency.is_empty() {
                return Err(FinanceError::Validation("Currency cannot be empty".to_string()));
            }
    ...
    pub fn transfer_funds(
        ...
        self.with_db(|db| {
            ...
            let tx_id = FinanceRepository::create_transfer(db, &from_id, &to_id, amount, &description, &date)?;
    ...
    pub fn add_transaction(
        ...
        self.with_db(|db| {
            ...
            if category.is_empty() {
                return Err(FinanceError::Validation("Category cannot be empty".to_string()));
            }
            ...
            if amount <= 0 {
                return Err(FinanceError::Validation("Amount must be greater than zero".to_string()));
            }
    ...
    pub fn get_balance(&self) -> Result<BalanceSummary, FinanceError> {
        self.with_db(|db| FinanceRepository::get_balance_summary(db).map_err(FinanceError::Database))
    }
    ...
    pub fn add_transaction_category(
        ...
        if validated_name.is_empty() {
            return Err(FinanceError::Validation("Category name cannot be empty".to_string()));
        }
        ...
        self.with_db(|db| {
            FinanceRepository::add_transaction_category(db, &validated_name, &category_type)
                .map_err(|e| match e {
    ...
    pub fn update_transaction_category(&self, id: String, new_name: String) -> Result<(), FinanceError> {
        let validated_id = validate_uuid(&id)?;
        ...
        self.with_db(|db| {
            FinanceRepository::update_transaction_category(db, &validated_id, &validated_name)
                .map_err(|e| match e {
                    DbError::Sqlite(RusqliteError::ExecuteReturnedResults) => {
                        FinanceError::Validation("Category with this name already exists".to_string())
                    }
    ...
    pub fn load_exchange_rate(&self, pair: String) -> Result<Option<(f64, String)>, FinanceError> {
        self.with_db(|db| {
            let cached = FinanceRepository::load_exchange_rate(db, &pair)
                .map_err(FinanceError::Database)?;
    ...
    pub fn get_analytics_summary(&self, range: String) -> Result<AnalyticsSummary, FinanceError> {
        ...
        let start_date = match range.as_str() {
            "1M" => today.checked_sub_signed(chrono::Duration::days(30)).unwrap_or(today),
            "3M" => today.checked_sub_signed(chrono::Duration::days(90)).unwrap_or(today),
            "6M" => today.checked_sub_signed(chrono::Duration::days(180)).unwrap_or(today),
            "1Y" => today.checked_sub_signed(chrono::Duration::days(365)).unwrap_or(today),
            _ => today,
        };
...
fn validate_field_length(value: &str, max_length: usize, field_name: &str) -> Result<String, FinanceError> {
...
fn validate_color(color: &str) -> Result<String, FinanceError> {
    ...
    if trimmed.len() != 7 {
        return Err(FinanceError::Validation("Color must be in #RRGGBB format".to_string()));
    }
    ...
    if !trimmed[1..].chars().all(|c| c.is_ascii_hexdigit()) {
        return Err(FinanceError::Validation("Color must contain valid hex characters".to_string()));
    }
