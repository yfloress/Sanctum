...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
        ...
        for (idx, (name, color, rate)) in chart_data.iter().enumerate() {
            ...
            right
                .draw(&Text::new(
                    label,
                    (swatch_x + 34, y + 8),
                    ("sans-serif", 42).into_font().color(&label_color),
                ))
                .ok()?;
        }
    ...
    fn render_habit_trend_chart(series: &[(String, String, Vec<f32>)]) -> Option<Image> {
        ...
        chart
            ...
            .label_style(("sans-serif", 42).into_font().color(&RGBColor(148, 163, 184)))
            .axis_style(ShapeStyle::from(&RGBColor(46, 46, 60)).stroke_width(1))
            .draw()
            .ok()?;
        ...
        chart
            ...
            .label_font(("sans-serif", 40).into_font().color(&RGBColor(148, 163, 184)))
